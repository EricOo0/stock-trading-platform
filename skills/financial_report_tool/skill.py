...
class FinancialReportSkill:
    ...
    def get_financial_indicators(self, symbol: str, years: int = 3, use_cache: bool = True) -> Dict[str, Any]:
        ...
        Returns:
            ...
            - profit: 利润端指标 (扣非归母净利EPS, 经营毛利率, 核心净利率)
    ...
    def _get_yf_symbol(self, market: str, normalized_symbol: str) -> str:
        """Helper method to get yfinance symbol with proper suffix"""
        if market == 'A-SHARE':
            if normalized_symbol.startswith('6'):
            ...
            return normalized_symbol if '.HK' in normalized_symbol else f"{normalized_symbol}.HK"
        else:
