...
class SimulationService:
    def __init__(self):
        self.agent_api_url = "http://localhost:8000/api/simulation/chat"
    ...
    async def run_daily_simulation(self, task_id: str, tools_instance) -> Dict:
        """
        ...
        try:
            # Direct Internal Call (No Requests/Network Overhead)
            from backend.app.services.fintech_service import fintech_service
            
            logger.info(f"Calling FintechService internally for session sim_{task_id}")
            agent_text = await fintech_service.chat_oneshot(
                query=prompt,
                user_id="simulation_system", 
                session_id=f"sim_{task_id}"
            )
