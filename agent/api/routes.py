"""API routes for the stock analysis agent using A2A protocol."""
...
async def chat(request: ChatRequest):
    ...
    try:
        ...
        # Run agent
        result = await agent.run(query=request.message, memory=memory)
...
async def update_config(request: ConfigUpdateRequest):
    ...
    try:
        ...
        return ConfigUpdateResponse(
            success=False, message=f"Failed to update configuration: {str(e)}"
        )
...
async def list_tools():
    """
    List all available A2A agents the system can use.
    """
    try:
        a2a_client = get_a2a_agent_client()
        ...
        # Get tool details from A2A agents
        tools_info = []
        for agent_name in agent_names:
            agent_info = a2a_client.get_agent_info(agent_name)
            if agent_info:
                tools_info.append(
                    ToolInfo(
                        name=agent_name,
...
async def health_check():
    ...
    try:
        ...
        return HealthResponse(
            status="healthy",
            version="0.2.0",  # Updated version for A2A support
            tools_loaded=len(agent.get_available_tools()),
        )
    except Exception as e:
